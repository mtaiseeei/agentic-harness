...
Do not infer or translate model names across hosts. Claude Code and Codex both inherit the user's current model and effort for every role by default. Do not ask Codex to identify itself as App or CLI. Codex CLI may omit `model`, `reasoning_effort`, and `agent_type` from its displayed spawn schema even when the runtime parser accepts them; schema omission alone must not force an explicitly configured value back to `inherit`. When native `spawn_agent` is available, dispatch the actual role once with the resolver's exact `dispatch-attempt` values. Use `agent_type`, never `agent_role`, to select a custom agent. Apply this rule to every exact model/effort selected by shared config, personal config, or the user, including explicit Luna/Sol settings.
...
`hosts.codex.custom_agents.enabled` is a separate application-path switch and defaults to `false`. When it is true and a Planner, Generator, or Evaluator resolves to exactly `gpt-5.6-luna`, use the resolver's `harness_luna_worker` custom-agent contract: pass `agent_type`, omit the model override, pass only a non-inherit resolved effort, and set `fork_turns: "none"`. This path is always fresh and is never resumed. A strong Generator decision happens first and goes directly to fresh Sol without trying Luna.
The resolver only inspects the global definition. If it reports `missing`, show the path and complete proposed TOML, obtain explicit user approval, and only then run `node "$PLUGIN_ROOT/scripts/provision-codex-agent.mjs" --approve`. If it reports `conflict`, never overwrite it; offer manual repair or `enabled = false`. After creating a definition, stop and continue in a new Codex task so the agent name is discovered.
